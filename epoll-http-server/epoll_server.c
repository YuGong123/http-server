...
void send_error(int cfd, int status, char *title, char *text)
{
	...
	// 状态行: 包括http协议版本号, 状态码, 状态信息。 消息报头: 说明客户端要使用的一些附加信息。 空  行: 必须!
	sprintf(buf, "%s %d %s\r\n", "HTTP/1.1", status, title);
	sprintf(buf+strlen(buf), "Content-Type:%s\r\n", "text/html");
	sprintf(buf+strlen(buf), "Content-Length:%d\r\n", -1);
	sprintf(buf+strlen(buf), "Connection: close\r\n");		//服务器完成一次和浏览器的通信，就主动断开链接
...
void epoll_run(int port)
{
    ...
    while(1) {
        ...
        int ret = epoll_wait(epfd, all, MAXSIZE, -1);	// 注意-1是阻塞，原来是-1；0是非阻塞。
        if(ret == -1) {
    ...
    for(i = 0; i < num; ++i) {
        ...
        if (ret == -1) {
            if (errno == EAGAIN) {
                ...
                perror("send error:");
                continue;
            } else if (errno == ECONNRESET) { // 对端重启，对端就要关闭。不能continue，会出bug。要break，要建立新的socket。
...
// 接受新连接处理，相当于客户端的 nc 127.0.0.1 9999,返回一个与客户端通信的cfd。
