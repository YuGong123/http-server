...
// 相当于客户端的 nc 127.0.0.1 9999,返回一个与客户端通信的cfd。
...
// 与客户端连接cfd读到了0,说明客户端已经关闭，或者断开了连接。
// 这时，需要先将cfd从红黑树上摘下，红黑树不用再监听cfd;然后关闭该cfd。
...
void http_request(int cfd, const char *file){
    ...
    if(S_ISREG(sbuf.st_mode)){  // 是一个普通文件
		...
		send_respond(cfd, 200, "OK", "audio/mpeg", sbuf.st_size);
...
void do_read(int cfd, int epfd)
{
	...
	if (len == 0) {
        ...
        // 上面只是读了一行数据，应该把剩下的数据读走。不然下次再读，有可能读取上一次遗留的数据。
		while (1) {
			char buf[1024] = {0};
